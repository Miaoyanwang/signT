...
\end{align}
where the second line comes from the boundedness of classification loss $L(\cdot ,\cdot)$, and the last line comes from the inequality $||\sign\tZ(\omega)-\sign\bar\tY_\Omega(\omega)|-|\sign\bar\Theta(\omega)-\sign \tY_\Omega(\omega)||\leq |\sign\tZ(\omega)-\sign\bar\Theta(\omega)|$ and the fact that the classification weight $|\bar\tY(\omega)|$ is bounded for $\omega\in\Omega$.
Here we ignore constant multiplication in $\leq$ because the constants will be suppressed in the asymptotical order relationship $\lesssim$ in the end.  The conclusion~\eqref{eq:variance} then directly follows by applying remark~\ref{eq:rmk} to~\eqref{eq:mae}.
\end{proof}
...
\]
We view $\tY_\Omega=\{\bar \tY(\omega)\colon \omega\in \Omega\}$ as a collection of $n$ independent random variables where the randomness is induced form both $\bar \tY$ and $\omega\sim\Pi$.  The tensor $\tZ$  induces a function $f_{\tZ}\colon \Omega\mapsto \mathbb{R	}$ such that $f_{\tZ}(\omega) = \tZ(\omega)$ for all $\omega\in\Omega$.  Then
\[
\Delta_n(f_{\tZ}, f_{\tY})
...
\end{align}
where the last line follows from the fact that $a(b^2+b^{(\alpha+2)/(\alpha+1)}+b+1) \leq 4 a (b^2+1)$ with $a:={t_n \over \rho^2}$ and $b:=\rho t_n^{-1/(\alpha+2)}$. Plugging the form of $t_n$ in~\eqref{eq:tn} completes the proof. 
\end{proof}
