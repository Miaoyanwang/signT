...
\begin{example}[Banded matrix] Let $\ma=(2^{-1},2^{-2},\ldots,2^{-d})^T\in\mathbb{R}^d$ and $\text{rev}(\ma)=(2^{-d},\ldots,2^{-1})^T\in\mathbb{R}^d$. We consider the following matrix
\[
\mM=\ma\otimes\ma+\text{rev}(\ma)\otimes \text{rev}(\ma).
\]
It is easy to see that the $\mM$ is both symmetric and skew symmetric,
\[
\mM(i,j)=\mM(j,i)=\mM(d-i,d-j)=\mM(d-j,d-i)=2^{-d-1}\left(2^{j-i}+2^{i-j}\right),\ \text{for all }(i,j)\in[d]^2.
\] 
Furthermore, $\rank(\mM)=2$, and $\sign(\mM-\pi)$ is a banded matrix. 
\end{example}
