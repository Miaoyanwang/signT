...
\tY=\Theta+\tE,
\end{equation}
where $\Theta\in\mathbb{R}^{d_1\times \cdots \times d_K}$ is an unknown signal tensor of interest, and $\tE$ is a noise tensor consisting of mean-zero, independently distributed entries. We allow heterogenous noise, in that the marginal distribution of noise entry $\tE(\omega)$ may depend on $\omega$. Assume that $\tY(\omega)$ takes value in a bounded interval $[-A, A]$; without loss of generality, we set $A=1$ throughout the paper.   We extend unbounded observation with sub-Gaussian noise in Appendix.
...
This section develops sign representable tensor models for $\Theta$ in~\eqref{eq:model}. We characterize the algebraic and statistical properties of sign tensor series, which serves the theoretical foundation. 
...
\]
\end{prop}
Conversely, the sign-rank can be much smaller than the tensor rank.
...
Some additional notation is needed.  We introduce a tolerance $\Delta s = 1/\prod_{i=1}^K d_i$  to account for discrete measure with finite tensor dimension.  Let $\tN$
be the set of mass points of $\Theta$ under $\Pi$, whose probability mass in a $\Delta s$-neighborhood  is heavier than the tolerance level; i.e.,
\begin{equation}\tN=\{\pi\colon\mathbb{P}_{\omega\sim \Pi}(|\Theta(\omega)-\pi|\leq \Delta s)> C\Delta s\}, \text{ for some universal constant } C>0.
\end{equation} 
 Note that both $\Pi$ and $\Theta$ implicitly depend on the tensor dimension. Our assumptions are imposed to $\Pi=\Pi(d)$ and $\Theta=\Theta(d)$ in the high-dimensional regime uniformly where $d:=\min_kd_k\to\infty$. 
...
\sup_{\Delta s\leq t<\rho(\pi, \tN)}{\mathbb{P}_{\omega \sim \Pi}[|\Theta (\omega)-\pi|\leq t ]\over t^\alpha} \leq c,
\end{equation}
...
The recovery is easier at levels where points are less concentrated around $\pi$ with a large value of $\alpha>1$, or equivalently, when the cumulative distribution function (CDF) $G(\pi):=\mathbb{P}_{\omega\sim \Pi}[\Theta(\omega)\leq \pi]$ remains almost  flat around $\pi$. A small value of $\alpha<1$ indicates the nonexistent (infinite) density at level $\pi$, or equivalently, when the $G(\pi)$ jumps by greater than the threshold level $\Delta s$ at $\pi$.  Table~\ref{tab:simulation} illustrates the $G(\pi)$ for various models of $\Theta$. 
...
The tensor block model is $\infty$-globally smooth. This is because the set $\tN$ consists of finite $\Delta s$-neighbors whose centers are distinct block means in $\Theta$. Furthermore, we have $\alpha= \infty$ for all $\pi \notin \tN$, since the numerator in~\eqref{eq:smooth} is zero.  Similarly, the min/max hypergaphon model such that  $\Theta(i_1,\ldots,i_K) = \log(1+\max_{\ell = 1,\ldots K}i_\ell/d)$ is $\infty$-globally smooth because $\alpha=\infty$ for all $\pi$ except points in $\tN$ consisting of  at most $d$ number of $\Delta s$-neighbors centering at $\log(1+i/d)$ for $i =1,\ldots, d$.
...
Let $|\tN|$ be the number of centers in $\tN$ defined as $|\tN| = \lceil \mu(\tN)/2\Delta s \rceil$, where $\mu$ is the Lebesgue measure. $|\tN|$ implies the possible levels in $\tH$ that cannot recover the sign signal.
...
\textup{MAE}(\hat \Theta, \Theta)\lesssim \left({d_{\max} r \over |\Omega|}\right)^{\alpha\over\alpha+2}+{|\tN|\vee 1\over H}+{Hd_{\max} r \over |\Omega|}.
\end{equation}
In particular, setting $\scriptstyle H\asymp \left( (|\tN|\vee 1)|\Omega|\over d_{\max}r\right)^{1/2}$ yields the error bound
\begin{equation}\label{eq:real}
\textup{MAE}(\hat \Theta, \Theta)\lesssim \max\left(\left(d_{\max}r \over |\Omega|\right)^{{\alpha \over \alpha+2} \wedge {1\over 2}},\left(|\tN|d_{\max}r\over |\Omega|\right)^{1\over 2}\right).
\end{equation}
\end{thm}
...
\begin{equation}
\textup{MAE}(\hat \Theta, \Theta)\lesssim\max\left( \left(r\over d^{K-1}\right)^{{\alpha \over \alpha+2} \vee {1\over 2}}, \left(|\tN|r\over d^{K-1}\right)^{1\over 2}\right).
\end{equation}
...
\caption{The full procedure of Algorithm 1}
\label{fig:alg}
\end{figure}
...
\begin{table}[h]
\includegraphics[width=1\textwidth]{cdf_combined_2.pdf}
