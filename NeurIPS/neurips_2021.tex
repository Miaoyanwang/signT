...
For simplicity, we assume the noise is bounded; the extension to a sub-Gaussian noise is provided in the supplement.  Here,  we assume the range of $\tY$ is the bounded interval $[-1,1]$ for cleaner exposition.
...
The results extend to general min/max hypergraphons. Let $g(\cdot)$ be a continuous univariate function with at most $r\geq 1$ distinct real roots in the equation $g(z)=\pi$; this property holds, e.g., when $g(z)$ is a polynomial of degree $r$. Then, the tensor $\Theta$ generated from $\Theta(i_1,\ldots,i_K)=g(\max_kx^{(k)}_{i_k})$ belongs to $\caliP(2r)$ (see the supplement). Same conclusion holds if max in $g(\cdot)$ is replaced by min. 
\end{example}


\subsection{Statistical characterization of sign tensors via weighted classification}\label{sec:identifiability}

Accurate estimation of a sign representable tensor depends on the behavior of sign tensor series, $\sign(\Theta-\pi)$. In this section, we show that sign tensors are characterized by weighted classification. The results bridge the algebraic and statistical properties of sign representable tensors.
...
\hat \Theta = {1\over 2H+1}\sum_{\pi \in \tH}\sign{\hat \tZ_\pi},
\end{equation}
where $\hat \tZ_\pi$ is a $\pi$-weighted classifier estimated at level $\pi \in \tH=\{-1,\ldots,-{1\over H}, 0, {1\over H},\ldots,1\}$,
