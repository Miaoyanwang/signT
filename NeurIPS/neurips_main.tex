...
\usepackage{xr}
\externaldocument{neurips_2021_supplement_v2}
...
\end{customexample}
Table~\ref{tb:comparison} summarizes the comparisons under different models in  Example 2-5. 

\begin{table}
\caption{Summary of our statistical rate compared to existing works under different models.  For notational simplicity, we present error rate assuming equal tensor dimension in all modes and $|\tN|$ being finite for the smooth tensor model. $K$: tensor order; $d$: dimension at each mode. }
\resizebox{\textwidth}{!}{
...
Single index model for $K = 2$              & 1        & Empty  & $-1/3$    & Improve previous rate -1/4 \citep{ganti2015matrix}          \\
Single index model for $K\geq3$ & 1        & Empty  &    $-(K-1)/3$                     & Not available                       \\
Generalized linear model                 & 1        & Empty  &       $-(K-1)/3$  & Close to minimax rate  \citep{zhang2018tensor,wang2018learning}             \\
Structure with repeating entries   & $\infty$   & $d$       &       $-(K-2)/2$    & Not available                       \\
Smooth tensor                                   & $\alpha$    &   Finite    &   $-(K-2)\min(\frac{\alpha}{\alpha+2},\frac{1}{2})$ & Not Avaiable      \\\hline                 
\end{tabular}
}
\label{tb:comparison}
\end{table}
...
Technical assumptions in the theorem depend on the chosen algorithm.  For example, a signal-to-noise ratio for base problem is needed for the polynomial complexity of algorithms in \cite{wang2018learning,han2020optimal}.
...
\begin{enumerate}
  \item Did you state the full set of assumptions of all theoretical results?
    \answerYes{The assumptions for main Theorems are fully stated in Sections~\ref{sec:identifiability} and~\ref{sec:error}. The assumptions for Theorem 4 and technical lemmas are stated in Appendix~\ref{sec:proofs}.}
...
\begin{enumerate}
  ...
  \item Did you specify all the training details (e.g., data splits, hyperparameters, how they were chosen)?
    ...
    \answerYes{Computational platform and running time are detailed in Appendix~\ref{sec:data}.}
\end{enumerate}
