...
\end{prop}
We provide several examples in Appendix, in which the tensor rank grows with dimension $d$ but the sign-rank remains a constant. The results highlight the advantages of using sign-rank in the high-dimensional tensor analysis. Proposition~\ref{cor:monotonic} and ~\ref{prop:extention} together demonstrate the strict broadness of low sign-rank family over the usual low-rank family. 
...
\textup{MAE}(\hat \Theta, \Theta)\lesssim \left({d_{\max} r\tilde K \over |\Omega|}\right)^{\alpha\over\alpha+2}+{1\over H}+{Hd_{\max} r\tilde K \over |\Omega|}.
\end{equation}
In particular, setting $\scriptstyle H\asymp \left( |\Omega|\over d_{\max}r\tilde K\right)^{1/2}$ yields the error bound
\begin{equation}\label{eq:real}
\textup{MAE}(\hat \Theta, \Theta)\lesssim \left(d_{\max}r\tilde K/|\Omega|\right)^{{\alpha \over \alpha+2} \vee {1\over 2}}.
\end{equation}
\end{thm}
\vspace{-.3cm}
...
\begin{equation}
\textup{MAE}(\hat \Theta, \Theta)\leq \left(r\tilde K/ d^{K-1}\right)^{{\alpha \over \alpha+2} \vee {1\over 2}}.
\end{equation}
...
\[
\textup{MAE}(\hat \Theta, \Theta)\to 0, \quad \text{as}\quad {|\Omega|\over {d_{\max}} r\tilde K}\to \infty.
\]
\end{cor}
\vspace{-.2cm}
Our result improves earlier work~\cite{yuan2016tensor,ghadermarzy2019near,pmlr-v119-lee20i} by allowing both low- and high-rank signals. Interestingly, the sample requirements depend only on the sign complexity $r$ but not the nonparametric complexity $\alpha$. Note that $\tilde \tO(d_{\max}r\tilde K)$ roughly matches the degree of freedom of sign tensors, suggesting the optimality of our sample requirements. 
...
\vspace{-.2cm}
\caption{MAE comparison between our method NonparaT ($H=20$) and CP low-rank method in the brain and NIPS data analysis. Standard errors are reported in parenthesis.}\label{tab:data}
\vspace{-.2cm}
\end{table}
