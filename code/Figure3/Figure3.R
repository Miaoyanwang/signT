...
save(error,error_con,file=paste("Figure3-dimension",d,".RData",sep=""))
...
pdf("Model3.pdf",width=4,height=3)
figure=ggplot(data,aes(dim,mean))+geom_point(aes(dim,mean,col=method,shape=method),size=2)+geom_line(aes(dim,mean,col=method))+geom_errorbar(aes(ymin=mean-se, ymax=mean+se,col=method), width=1,position=position_dodge(0))+labs(x = "dimension")+labs(y = "MAE")+coord_cartesian(ylim = c(0.01, 0.04))
