...
figure=ggplot(data,aes(x=dim,y=mean))+geom_point(aes(x=dim,y=mean,col=method,shape=method),size=2)+geom_line(aes(x=dim,y=mean,col=method))+geom_errorbar(aes(ymin=mean-se/sqrt(30), ymax=mean+se/sqrt(30),col=method), width=.01,position=position_dodge(0))+labs(x = "Observation faction")+labs(y = "MAE")+coord_cartesian(ylim = c(0, 0.2))
