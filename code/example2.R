...
type=as.factor(c(rep("Z",30),rep("T",30)))
data[,2]=log10(data[,2])
data=data.frame(data,type)

pdf("example2.pdf",width=4,height=3)
ggplot(data,aes(component,singular.value))+geom_point(aes(component,singular.value,shape=type,col=type))+ylim(2.5,4.5)
dev.off()
