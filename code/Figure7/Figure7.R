...
save(error,error_con,file=paste("Figure7-dimension",rho,".RData",sep=""))
...
rho_list=seq(from=0.3,to=1,length=8)
con=ours=NULL
load("Figure7-dimension0.3.RData")
ours=error; con=error_con
for(i in 2:7){
    load(sprintf("Figure7-dimension%.1f.RData",rho_list[i]))
    ours=rbind(ours,error)
    con=rbind(con,error_con)
}
load("Figure7-dimension1.RData")
ours=rbind(ours,error); con=rbind(con,error_con)
error=ours;error_con=con
save(error,error_con,file="Model7.RData")
#######################
...
pdf("Model7.pdf",width=4,height=3)
