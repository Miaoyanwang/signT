...
print(paste("dimension",rho,"-simulation",sim," is done",sep = ""))
}

save(error,error_con,file=paste("Figure6-dimension",rho,".RData",sep=""))

####################### load output from server#######################
rho_list=seq(from=0.3,to=1,length=8)
con=ours=NULL
load("Figure6-dimension0.3.RData")
ours=error; con=error_con
for(i in 2:7){
    load(sprintf("Figure6-dimension%.1f.RData",rho_list[i]))
    ours=rbind(ours,error)
    con=rbind(con,error_con)
}
load("Figure6-dimension1.RData")
ours=rbind(ours,error); con=rbind(con,error_con)
error=ours;error_con=con
save(error,error_con,file="Model6.RData")
#######################
...
pdf("Model6.pdf",width=4,height=3)
...
source("../signT.R")
signal=graphon_to_tensor(a,a,a,type=6)
pdf("cdf_model2.pdf",height=5,width=10)
Ecdf(c(signal),xlim=c(min(signal)-0.5,max(signal)+0.5))
abline(h=0,lty=2)
abline(h=1,lty=2)
dev.off()
