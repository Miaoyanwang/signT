...
topword=dimnames(denoise)[[2]][c(1,6,8,11,12,15,16,17,18,19)]

### after denoising
pdf("after_denoise.pdf",width=10,height=10)
levelplot(denoise[1:10,rev(topword),c(3,4,2,1)],col.regions=hcl.colors(100, "YlOrRd", rev = TRUE),at=seq(0,5, length.out=100),scales=list(x=list(rot=45))) ## reverse the word order b.c. default layout in levelplot
dev.off()
### before denoising
pdf("before_denoise.pdf",width=10,height=10)
levelplot(tensor[1:10,rev(topword),c(3,4,2,1)],col.regions=hcl.colors(100, "YlOrRd", rev = TRUE),at=seq(0,5, length.out=100),scales=list(x=list(rot=45)))
dev.off()
