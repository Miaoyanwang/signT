...
\appendix
The appendix consists of  numerical experiments (Section~\ref{sec:data}),  proofs (Section~\ref{sec:proof}), and additional theoretical results (Section~\ref{sec:additional}).

\section{Additional results on numerical experiments}~\label{sec:data}
\vspace{-1cm}
\subsection{Simulation models used in the main paper}
We have described four simulation models in the main paper. Unfortunately, the schematic figures in Table 1 were not displayed properly. Here we provide a high resolution version again for better repository.
\begin{table*}[h!]
...
\subsection{Extension of Theorems~\ref{thm:classification} and~\ref{thm:estimation} to unbounded observation with sub-Gaussian noise}
...
\begin{align}
 \textup{MAE}(\textup{sgn}(\hat \tZ_\pi),\textup{sgn}(\Theta-\pi))\lesssim \left(r\sigma^2d_{\max}\log d_{\max}\over |\Omega|\right)^{\alpha\over \alpha+2}+\frac{1}{\rho^2(\pi,\tN)}{r\sigma^2d_{\max}\log d_{\max}\over |\Omega|}.
\end{align}
\end{thm}
...
\end{itemize}
Because the Event I occurs with probability tending to zero, we restrict ourselves to the Event II only by following the proof of Theorem~\ref{thm:classification}.   We summarize the key difference compared to Section~\ref{sec:proof}. 
We expand the variance by 
\begin{align}
    \label{eq:variance2}
...
\end{align}
In particular, setting $H\asymp \left({ |\Omega|\over r \sigma^2d_{\max}\log d_{\max}}\right)^{1/2}$ yields the error bound
\begin{align*}
 \textup{MAE}(\hat \Theta,\Theta)\lesssim \left({r \sigma^2d_{\max}\log d_{\max}\over |\Omega|}\right)^{\min({\alpha\over\alpha+2},\frac{1}{2})}.
\end{align*}
\end{thm}
