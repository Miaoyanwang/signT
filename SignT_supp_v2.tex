...
\begin{align}
 \text{MAE}(\text{sgn}(\hat \tZ),\text{sgn}(\bar\Theta))\lesssim \left(r\sigma^2d_{\max}\log d_{\max}\over |\Omega|\right)^{\alpha\over \alpha+2}+\frac{1}{\rho^2(\pi,\tN)}{r\sigma^2d_{\max}\log d_{\max}\over |\Omega|}.
\end{align}
\end{thm}
