...
\date{}
\author{%
Chanwoo Lee$^{1}$ and Miaoyan Wang$^{2}$ \\
Department of Statistics, University of Wisconsin-Madison 
\footnote{\hspace{-.5cm}$^1$ chanwoo.lee@wisc.edu\quad $^2$ miaoyan.wang@wisc.edu}
}
...
Like most tensor problems~\citep{hillar2013most}, determining the sign-rank for a general tensor is NP hard~\citep{alon2016sign}. Fortunately, tensors arisen in applications often possess special structures that facilitate analysis. %We show that the low-sign-rank tensor family is strictly broader than usual low-rank family. 
...
We apply our method to two tensor datasets, the MRN-114 human brain connectivity data~\citep{wang2017bayesian}, and NIPS data~\citep{globerson2007euclidean}. 
...
\label{fig:braincv}
\end{figure}
Figure~\ref{fig:braincv} compares the prediction accuracy of different methods across rank $r = 3,6,9,12,15$ with four different missing rates.
MAEs are averaged over five runs of cross-validation and Table~\ref{tab:data} reports the part of  Figure~\ref{fig:braincv} when missing rate is 20\% and rank = 6,9,12.
We find that our method outperforms substantially in all combinations of ranks and missing rates.   Figure~\ref{fig:braincv}  shows that the performance gap between two methods increases as the proportion of  missing data gets higher.  This  trend  implies  that increased missingness gives more advantages to our method.  In addition, Table~\ref{tab:data} shows that  our method has smaller standard errors of MAE from five repeated cross-validation  which implies more stable performance over CPT.  One advantage of our method is that our estimation is always bounded in $[0,1]$ whereas CPT estimation may fall outside the valid range $[0,1]$. 
...
\hline
NonparaT (Ours)& {\bf 0.18}(0.000) & {\bf 0.14}(0.000) &  {\bf 0.12}(0.000)&{\bf 0.12}(0.000)&{\bf 0.11}(0.000)\\
CPT &0.26(0.006) & 0.23(0.006)&0.22(0.004)&0.21(0.006)&0.20(0.008)\\
 \Xhline{2\arrayrulewidth}
\end{tabular}
}
\caption{MAE comparison between our method {\bf NonparaT} ($H=20$) and CP low-rank tensor method ({\bf CPT}) in the brain data analysis. Standard errors are reported in parenthesis.}\label{tab:data}
\end{table}

We next examine the estimated signal tensor $\hat\Theta$ from our method with the observed binary tensor $\tY,r = 10$, and $H =  20$. We regress the denoised connection strength of brain edge $(i,j)$ across 114 individuals ($\hat\Theta(i,j,)$) to normalized IQ scores.  We repeated the regression for every brain edge.  We plot the top 10 brain edges based on the regression analysis in Figure~\ref{fig:signal}a .
We find that top connections are mostly inter-hemisphere edges, consistent with recent research on brain connectivity with intelligence~\citep{li2009brain,wang2017bayesian}.
...
\begin{figure}[h]
\centering
\includegraphics[width=.295\textwidth]{figure/brainIQ.pdf}
\includegraphics[width=.395\textwidth]{figure/signal.pdf}
...
Table~\ref{tab:NIPS} compares the prediction accuracy of different methods. Reported MAEs are averaged over five runs of cross-validation, with 20\% entries for testing and 80\% for training. 
In addition, we also perform naive imputation where the missing values are predicted using the observed sample average for comparison.  Both our method and CPT outperform the naive imputation, implying the necessity of incorporating tensor structure in the analysis.
We find that our method substantially outperforms the low-rank CP method for every configuration under consideration. Further increment of rank appears to have little effect on the performance. 
The comparison highlights the advantage of our method in achieving accuracy while maintaining low complexity. 
...
The appendix consists of additional theoretical results (Section~\ref{sec:additional}) and proofs (Section~\ref{sec:proof}). 
...
\[
\rank(\Theta)=\rank(\mM),\quad \text{and}\quad \srank(\Theta-\pi)\leq\srank(\mM-\pi) \ \text{for all $\pi\in\mathbb{R}$}.
\] 
\end{prop}
\begin{proof}
