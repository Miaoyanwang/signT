...
\subsection{Extension of Theorem~\ref{thm:classification} and~\ref{thm:estimation} to unbounded observation case}
...
\item The noise entries $\tE(\omega)$ are independent and sub-Gaussian, i.e, $\mathbb{P}(|\tE(\omega)|\geq B)\leq Ce^{-B^2/\sigma^2}$ for $B>0.$
\end{enumerate}
\end{assumption}
\begin{thm}[Sign tensor estimation for unbounded observation case]\label{thm:unbddno1}
Consider the same condition of Theorem~\ref{thm:classification}.   Suppose that Assumption~\ref{assm:subg} holds. Then, for all $\pi\in[-1,1]$ except for a finite number of levels,  with the probability  $1-c\exp\left(-\sigma^2rd_{\max}\log d_{\max}\right)-\frac{1}{d_{\max}},$
\begin{align}
...
\end{align}
where the last line comes from the definition of MAE and the inequality $\|\tE\|_\infty \geq 2$ with high probability.
...
\end{align}
where the convergence rate $L_n$ is obtained by the same way in the proof of Lemma~\ref{lem:metric} as 
\begin{align}
L_n\asymp t_n^{(\alpha+1)/(\alpha+2)}+\frac{1}{\rho}t_n,\quad\text{ where } t_n =  {\|\tE\|^2_\infty d_{\max}r K\log K\over n}.
\end{align}
Plugging $t = d_{\max}$ into Lemma~\ref{lem:subg},  we have 
\begin{align}\label{eq:subgcont}\|\tE\|_\infty\leq \sqrt{2\sigma^2(K+1)\log d_{\max}},\end{align}
with the probability $1-1/d_{\max}$.
...
\end{align} with the probability $1-c\exp\left(-\sigma^2rd_{\max}\log d_{\max}\right)-1/d_{\max}$ 
for some constant $c>0$.
Therefore,  combining \eqref{eq:riskunbd} and \eqref{eq:final} completes the proof.
\end{proof}

  We obtain tensor estimation error for unbounded observation case from the proof of Theorem~\ref{thm:estimation} and Theorem~\ref{thm:unbddno1}.
\begin{thm}[Tensor estimation error for unbounded observation case] Consider the same conditions of Theorem~\ref{thm:unbddno1}. With the probability  $1-c\exp\left(-\sigma^2rd_{\max}\log d_{\max}\right)-\frac{1}{d_{\max}},$
\begin{align}
...
Let $X_1,\ldots,X_n$ be independent sub-Gaussian random variables with $\sigma^2$. Then, for any $t>0,$
\[\mathbb{P}\left\{\max_{1\leq i\leq n}|X_i|\geq\sqrt{2\sigma^2(\log n +t)}\right\}\leq2 e^{-t}.\]
\end{lem}
\begin{proof}
Notice that 
\begin{align}
\mathbb{P}[\max_{1\leq i\leq n}X_i\geq u] \leq \sum_{i=1}^n\mathbb{P}[X_i\leq u]\leq n e^{-{u^2\over 2\sigma^2}} = e^{-t},
\end{align}
